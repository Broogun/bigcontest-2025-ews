...
그러나 이 시스템의 실제 운영 맥락을 고려하면 LightGBM이 올바른 선택입니다.

#### 1. 이 시스템에서 가장 중요한 지표는 AUC가 아닌 Lift@5%

조기경보 시스템의 실전 사용 방식은 다음과 같습니다.
...
AUC는 4,183개 전체 순위의 품질을 측정하지만, 현장에서는 **상위 소수에 자원을 집중 투입**합니다. 이 맥락에서 Lift@5%가 더 직접적인 성능 지표입니다.
...
조기경보 목적에서 LightGBM이 현장 투입 효율이 더 높습니다.

#### 2. CatBoost의 AUC 우위는 통계적으로 유의하지 않음
...
폐업이 30개뿐인 극소표본에서 5-Fold CV AUC 0.012 차이는 랜덤 시드나 데이터 분할 방식에 따라 얼마든지 역전됩니다. CatBoost의 Holdout AUC가 CV보다 낮은 것(Δ=-0.061)도 경미한 과대 추정 신호입니다.

#### 3. 피처 수정 전·후 이력

처음 16개 피처(`rank_ind_trend`, `rank_dist_trend` 누락)로 실행했을 때는 LightGBM이 명확한 1위(AUC 0.797)였고, CatBoost는 0.770으로 격차가 컸습니다. 피처 버그 수정 후 두 피처가 CatBoost에 특히 도움이 되어 격차가 0.012로 좁혀진 것으로, CatBoost가 해당 피처 유형에 민감하게 반응하는 구조임을 시사합니다. 이 점이 오히려 CatBoost의 일반화 안정성에 대한 추가 검토 필요성을 높입니다.
