...
### LightGBM · SHAP · EWS 두 관점 진단 + GPT-4o-mini AI 경영 진단 리포트
...
15. [GPT-4o-mini AI 경영 진단 리포트](#15-gpt-4o-mini-ai-경영-진단-리포트)
...
4. **LLM 기반 실무 연계** — LightGBM·SHAP·EWS의 정량 결과를 GPT-4o-mini가 자연어 경영 진단 리포트로 변환 (진단 요약 → 관측 → 가설 해석 → 단계별 실행 계획 → 맞춤 금융상품 추천)
...
### 8.1 탐지 트랙: LightGBM + SHAP 알고리즘 (관점 1)

`dw_f_*` 18개 피처 → RandomizedSearchCV(n_iter=40) 하이퍼파라미터 최적화 → CV AUC 0.798 최종 선택.  
학습 후 SHAP 그룹 합산(`shap_int/comp/ext_pct`)으로 예측 근거 투명화.  
상세 구성은 [§8.2 산출식](#최종-점수-산출식) 참고.
...
```
                CV AUC   Lift@5%  Lift@10%   Holdout AUC    Δ (Holdout-CV)
LightGBM ★      0.798     7.3x     4.3x       넓은 CI       —
CatBoost        0.810     6.7x     4.0x        0.748       -0.061 (낙관적)
```

**Lift@5% / @10% 차이의 실체**

```
상위 5%  = 4,183 × 0.05 ≈ 209개   →  LGB 7.3x (10~11개) vs CB 6.7x (9~10개): 1개 차이
상위 10% = 4,183 × 0.10 ≈ 418개   →  LGB 4.3x (13개)    vs CB 4.0x (12개):  1개 차이
```

두 구간 모두 LightGBM이 앞서며, 차이는 폐업 1개 수준이다. 조기경보 목적에서 LightGBM이 현장 투입 효율이 더 높다.
...
- **현장 연계**: AI 리포트(GPT-4o-mini)와 맞춤 금융상품 매칭으로 탐지 → 대응까지 원스톱 처리
...
| 💳 맞춤 금융 서비스 | 등급별 금융상품 자동 매칭 + GPT-4o-mini AI 경영 진단 리포트 |
