...
15. [대시보드](#15-대시보드-로컬-실행)
16. [프로젝트 구조](#16-프로젝트-구조)
17. [데이터 및 환경](#17-데이터-및-환경)
18. [참고문헌](#18-참고문헌)
...
- **현장 연계**: AI 리포트(Claude Haiku)와 맞춤 금융상품 매칭으로 탐지 → 대응까지 원스톱 처리
...
1. **다년도 검증**: 3년 이상 데이터를 통한 시간적 일반화 능력 검증 및 파라미터 재최적화
2. **실시간 갱신 파이프라인**: 월별 카드사 데이터 자동 연동 → 점포별 위험 등급 월간 갱신
3. **타 지역·업종 적용**: 성동구 외 타 상권 적용 가능성 검증 및 이식성 연구
4. **연속 매출 데이터 연계**: 버킷값 대신 실제 매출 금액 활용 시 정밀도 개선 가능성 탐색
5. **능동 학습 적용**: 극소 표본 문제 완화를 위한 Active Learning 방법론 검토
